...
import style from "./CurrentProducts.module.css";
...
export default function CurrentProducts() {
 ...
 return<div className="flex items-center justify-center h-screen">
  <span className="text-textPink text-[30px] "><i class="fa-regular fa-circle-xmark"></i> Please try again later</span>
</div>  
 }
    ...
    <>
      {
        ...
        <div className="row ">
          {data?.map((product) => (
             <div key={product.id} className="lg:w-1/4  w-full md:w-1/2 mb-3 p-3   ">
              <div className="product  py-2 px-5 text-center bg-transparent shadow-lg">
                ...
                >
                  <img src={product.imageCover} className="w-[100px] mx-auto " alt="" />
                ...
                <button onClick={()=>{NewAddCart(product.id)}} className="btn">
                  {
                  loading && btnId==product.id ? <span className="cartloader"></span>: `Add to cart`
                  }
                  </button>
              </div>
            </div>
