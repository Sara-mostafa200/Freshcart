import style from "./VerifyPass.module.css";
...
export default function VerifyPass() {
  ...
  let { verifyPass } = useContext(UserContext);

  async function handleVerifyPassword(values) {
    console.log(values);

    let response = await verifyPass(values);
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="max-w-md mx-auto">
      ...
      <div className="text-start">
        <Link to="/login">
          {" "}
          <p className=" mt-3 underline text-textGray font-medium text-[15px] ">
